...
const createNotationCommentMarkerRule = (parser, { classMap, classPre, notation }) => {
    ...
    `\\s*(?://|/\\*|<!--|#|--)\\s+\\[!code (${marker})(:\\d+)?\\]\\s*(?:\\*/|-->)?`), ([, match, range = ':1'], index) => {
        ...
        parser.line((line, i) => {
            ...
            if (parser.lang === 'diff' && notation === 'focus') {
                line.content = line.content.replaceAll('class="', `class="${toArray(classMap[match]).join(' ')} `);
            }
        });
        if (classPre) {
