...
const SpeechRules = __importStar(require("../speech_rules/speech_rules.js"));
const SpeechRuleStores = __importStar(require("../speech_rules/speech_rule_stores.js"));
...
const speech_structure_js_1 = require("./speech_structure.js");
class SpeechRuleEngine {
    ...
    evaluateNode(node, _clear = false) {
        (0, xpath_util_js_1.updateEvaluator)(node);
        if (!this.speechStructure || _clear) {
            this.speechStructure = new speech_structure_js_1.SpeechStructure(node);
        }
    ...
    runInSetting(settings, callback) {
        ...
        const save = engine.options;
        const newOpt = new options_js_1.Options();
        for (const key of options_js_1.Options.BINARY_FEATURES) {
            newOpt[key] = (_a = settings[key]) !== null && _a !== void 0 ? _a : save[key];
        }
        for (const key of options_js_1.Options.STRING_FEATURES) {
            newOpt[key] = (_b = settings[key]) !== null && _b !== void 0 ? _b : save[key];
        }
    ...
    evaluateTree_(node) {
        ...
        this.speechStructure.addNode(node, result, engine_js_1.Engine.getInstance().options.modality);
        return result;
    }
    evaluateTreeInternal_(node) {
        const engine = engine_js_1.Engine.getInstance();
        let result;
        debugger_js_1.Debugger.getInstance().generate(() => [node.toString()]);
    ...
    updateConstraint_() {
        ...
        const strict = engine_js_1.Engine.getInstance().options.strict;
...
//# sourceMappingURL=speech_rule_engine.js.map